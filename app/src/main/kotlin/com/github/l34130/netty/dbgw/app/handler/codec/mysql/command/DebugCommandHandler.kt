...
class DebugCommandHandler(
    ...
    override fun channelRead0(
        ...
        if (msg.payload.peek { it.readFixedLengthInteger(1) } == 0x0DUL) {
            ...
            proxyContext.upstream().pipeline().addBefore(
                ...
                DebugCommandResponseHandler(proxyContext),
            )
    ...
    private class DebugCommandResponseHandler(
        private val proxyContext: ProxyContext,
    ) : SimpleChannelInboundHandler<Packet>() {
        override fun channelRead0(
            ctx: ChannelHandlerContext,
            ...
            when {
                msg.isOkPacket() -> {
                    logger.trace {
                        ...
                        "Received COM_DEBUG response: ${Packet.Ok.readFrom(msg.payload, proxyContext.capabilities())}"
                ...
                msg.isErrorPacket() -> {
                    logger.trace {
                        msg.payload.markReaderIndex()
                        "Received COM_DEBUG response: ${Packet.Error.readFrom(msg.payload, proxyContext.capabilities())}"
                        msg.payload.resetReaderIndex()
                    }
                }
                else -> {
