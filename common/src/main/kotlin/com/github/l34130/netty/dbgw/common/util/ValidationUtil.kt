...
object ValidationUtil {
    ...
    private val DNS1123_LABEL_REGEX = Regex("^$DNS1123_LABEL_FMT$")
    private val DNS1123_SUBDOMAIN_REGEX = Regex("^$DNS1123_SUBDOMAIN_FMT$")
    ...
    fun validatePascalCase(value: String): List<Violation> {
        ...
        if (value.first().isLowerCase() || value.last().isLowerCase()) {
            violations.add("must start and end with an uppercase letter or digit")
        }
        return violations
    }
    ...
    fun validateRFC1123DnsLabel(value: String): List<Violation> {
        ...
        if (!DNS1123_LABEL_REGEX.matches(value)) {
            violations.add(
    ...
    fun validateRFC1123DnsSubdomain(value: String): List<Violation> {
        ...
        if (!DNS1123_SUBDOMAIN_REGEX.matches(value)) {
            violations.add(
